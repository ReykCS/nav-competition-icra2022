...
{   
    ...
    goal_sub_ = public_nh.subscribe("global_goal", 1, &SpacialHorizon::goalCallback,this);

    public_nh.getParam("/real", is_real);
    if(is_real){
        amcl_pose_sub_ = public_nh.subscribe("amcl_pose", 1, &SpacialHorizon::amcl_poseCallback, this);
    }else{
        odom_sub_ = public_nh.subscribe("odometry/filtered", 1, &SpacialHorizon::odomCallback, this);
    }
